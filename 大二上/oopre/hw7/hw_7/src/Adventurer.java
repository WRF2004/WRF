...
public class Adventurer implements Commodity {
    ...
    public void moneyCheck() {
        if (this.hitPoint <= this.preHitpoint / 2) {
            long price = (this.preHitpoint - this.hitPoint) * 10000L;
            for (Adventurer obj : adventurers) {
                if (obj.showMoney() >= price) {
                    obj.subMoney(price);
                    this.money += price;
                } else {
                    this.money += obj.showMoney();
                    obj.subMoney(obj.showMoney());
                }
            }
        }
        this.preHitpoint = this.hitPoint;
    }

    public HashSet<Bottle> getBottleBag() { return bottleBag; }

    public HashSet<Equipment> getEquipmentBag() { return equipmentBag; }

    public HashSet<Food> getFoodBag() { return foodBag; }

    public long showMoney() { return this.money; }

    public void subMoney(long money) { this.money -= money; }

    public long clearBag() {
        long sum = 0;
        Iterator<Bottle> bottleIterator = bottleBag.iterator();
        while (bottleIterator.hasNext()) {
            Bottle bottle = bottleIterator.next();
            bottles.remove(bottle.showId(), bottle);
            this.count -= 1;
            commodities.remove(bottle);
            sum += bottle.showPrice();
            bottleIterator.remove();
        }
        Iterator<Food> foodIterator = foodBag.iterator();
        while (foodIterator.hasNext()) {
            Food food = foodIterator.next();
            foods.remove(food.showId(), food);
            commodities.remove(food);
            this.count -= 1;
            sum += food.showPrice();
            foodIterator.remove();
        }
        Iterator<Equipment> equipmentIterator = equipmentBag.iterator();
        while (equipmentIterator.hasNext()) {
            Equipment equipment = equipmentIterator.next();
            equipments.remove(equipment.showId(), equipment);
            commodities.remove(equipment);
            this.count -= 1;
            sum += equipment.showPrice();
    ...
    public String showType() { return this.type; }

    public long showMaxPrice() { return this.maxPrice; }

    public int showCount() { return this.count; }

    public int showId() { return this.id; }

    public String showName() { return this.name; }

    public int showAttr() { return this.level; }
    ...
    public void addBot(int botId, String name, int capacity, long price, String type, double o) {
    ...
    public Bottle subBot(int botId) {
        Bottle bottle = bottles.get(botId);
        this.money += bottle.showPrice();
    ...
    public void addEqu(int equId, String name, int star, long price, String type, double o) {
    ...
    public Equipment subEqu(int equId) {
        Equipment equipment = equipments.get(equId);
        if (equipment != null) {
            this.count -= 1;
            this.money += equipment.showPrice();
    ...
    public void addFood(int foodId, String name, int energy, long price) {
    ...
    public Food subFood(int foodId) {
        Food food = foods.get(foodId);
        this.count -= 1;
        this.money += food.showPrice();
        foods.remove(foodId);
        foodBag.remove(food);
        commodities.remove(food);
        return food;
    ...
    public Bottle bottleCheck(String name) {
        Bottle bottle = null;
        for (Bottle obj : bottleBag) {
            if (Objects.equals(obj.showName(), name)) {
                if (bottle == null) {
    ...
    public void fightUseBot(String name) {
        Bottle bottle = bottleCheck(name);
        int botId = bottle.showId();
        useBottle(bottle);
